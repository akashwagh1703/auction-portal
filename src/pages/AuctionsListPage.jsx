  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        {isAdmin && (
          <Button onClick={() => { setModal(true); setStep(1); setForm(defaultForm) }}>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={`Create Auction — Step ${step} of 3`}>
        <div className="flex gap-2 mb-6">
          {['Basic Info', 'Settings', 'Players & Teams'].map((label, i) => (
            <div key={i} className="flex-1 text-center">
              <div className={`h-1.5 rounded-full mb-1.5 transition-all ${step > i ? 'bg-blue-500' : step === i + 1 ? 'bg-blue-500' : 'bg-slate-700'}`} />
        ...
        {step === 2 && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <Input label="Bid Timer (seconds)" type="text" inputMode="numeric" min="10" max="120" value={form.bid_timer} onChange={e => { if (/^\d*$/.test(e.target.value)) setField('bid_timer', e.target.value) }} />
            ...
            <Input label="Max Players Per Team" type="text" inputMode="numeric" min="1" value={form.max_players_per_team} onChange={e => { if (/^\d*$/.test(e.target.value)) setField('max_players_per_team', e.target.value) }} />
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <button
                  onClick={() => setForm(f => ({
                    ...f,
                    player_ids: f.player_ids.length === players.length ? [] : players.map(p => p.id)
                  }))}
